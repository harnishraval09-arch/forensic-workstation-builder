...
# Primary UI typeface (falls back gracefully if Inter isn't installed
# on the user's system - Segoe UI/Roboto look close enough that the
# fallback isn't jarring).
FONT_SANS = '"Inter", "Segoe UI", "Roboto", "Helvetica Neue", Arial, sans-serif'


# Used for file paths and anything log/console-like, so backslashes
# and folder names line up in fixed-width columns.
FONT_MONO = '"JetBrains Mono", "Roboto Mono", "Consolas", "Courier New", monospace'
...
def build_stylesheet(theme_name: str) -> str:
    ...
    QPushButton#PrimaryButton {{
        background-color: qlineargradient(
            x1:0, y1:0, x2:1, y2:1,
            stop:0 {p.primary}, stop:1 {p.header_grad_start}
        );
        color: #ffffff;
        border: none;
        padding: 9px 20px;
    ...
    QPushButton#PrimaryButton:hover {{
        background-color: qlineargradient(
            x1:0, y1:0, x2:1, y2:1,
            stop:0 {p.primary_hover}, stop:1 {p.header_grad_start}
        );
        border: none;
    }}
