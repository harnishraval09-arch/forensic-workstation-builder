...
PAGE_TITLES = {
    "dashboard": (
        "Dashboard",
        "Overview of your forensic workstation environment"
    ),
    "catalogue": (
        "Tool Catalogue",
        "Browse and install forensic tools"
    ),
    "profiles": (
        "Profiles",
        "Curated environment bundles"
    ),
    "jobs": (
        "Jobs & Installations",
    ...
    "audit": (
        "Audit Log",
        "Full trail of installs, uninstalls, and configuration changes"
    ),
    "settings": (
        "Settings",
        "Configure directories, theme, and logging"
    ),
}
...
class MainWindow(QMainWindow):
    def __init__(self, app, app_state):
        ...
        for key, icon, label in NAV_ITEMS:
            ...
            btn.clicked.connect(
                lambda checked, k=key: self.navigate(k)
            )
        ...
        self.pages = {
            ...
            "audit": AuditPage(self.app_state),
        ...
        self.app_state.data_changed.connect(self._update_status_pill)
    ...
    def _update_status_pill(self):
        ...
        total = len(tm.tools)
        installed = len(tm.installed_tools)

        self.status_pill.setText(
            f"●  {installed}/{total} tools installed"
        )
