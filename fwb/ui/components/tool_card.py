...
CATEGORY_COLORS = {
    "Digital Forensics": "#3498db",
    "Malware Analysis": "#e74c3c",
    "Network Forensics": "#9b59b6",
    "General": "#1abc9c",
}
...
class ToolCard(QFrame):
    ...
    def __init__(self, tool, parent=None):
        ...
        top_row = QHBoxLayout()
        icon = QLabel(_initials(tool.name))
        icon.setFixedSize(40, 40)
        icon.setAlignment(Qt.AlignCenter)
        icon.setStyleSheet(
            f"background-color: rgba(52,152,219,0.15); color: {accent};"
        ...
        details_btn.clicked.connect(lambda: self.clicked.emit(self.tool.id))
