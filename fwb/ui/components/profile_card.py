...
class ProfileCard(QFrame):
    ...
    def __init__(self, profile, tool_count: int, installed_count: int, parent=None):
        ...
        icon.setStyleSheet(
            "background-color: rgba(52,152,219,0.15); border-radius: 10px; font-size: 19px;"
        )
        ...
        progress_label = QLabel(f"{installed_count} / {tool_count} tools installed")
        progress_label.setObjectName("Muted")
        root.addWidget(progress_label)

        btn_row = QHBoxLayout()
        install_btn = QPushButton(
            "Install Profile" if installed_count < tool_count else "Reinstall Profile"
        )
        install_btn.setObjectName("PrimaryButton")
        install_btn.clicked.connect(lambda: self.install_requested.emit(self.profile.id))
